...
use crate::framebuffer::{Framebuffer, Pixmap, Samples, ToSamples, UpdateMode};
...
impl Dictionary {
    pub fn new(rect: Rectangle, query: &str, language: &str, hub: &Hub, rq: &mut RenderQueue, context: &mut Context) -> Dictionary {
        ...
        let image = Image::new(image_rect, Pixmap::new(1, 1, Samples::Grey));
    ...
    fn go_to_neighbor(&mut self, dir: CycleDir, rq: &mut RenderQueue) {
        ...
        if let Some(image) = self.children[4].downcast_mut::<Image>() {
            if let Some((pixmap, loc)) = self.doc.pixmap(location, 1.0, CURRENT_DEVICE.to_samples()) {
    ...
    fn define(&mut self, text: Option<&str>, rq: &mut RenderQueue, context: &mut Context) {
        ...
        if let Some(image) = self.children[4].downcast_mut::<Image>() {
            if let Some((pixmap, loc)) = self.doc.pixmap(Location::Exact(0), 1.0, CURRENT_DEVICE.to_samples()) {
                image.update(pixmap, rq);
                self.location = loc;
            }
        }
...
impl View for Dictionary {
    ...
    fn resize(&mut self, rect: Rectangle, hub: &Hub, rq: &mut RenderQueue, context: &mut Context) {
        ...
        if let Some(image) = self.children[4].downcast_mut::<Image>() {
            if let Some((pixmap, loc)) = self.doc.pixmap(Location::Exact(self.location), 1.0, CURRENT_DEVICE.to_samples()) {
