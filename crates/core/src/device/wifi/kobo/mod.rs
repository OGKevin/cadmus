...
//! ## Why No DHCP Client?
//!
//! Unlike the original shell script, this implementation does NOT run `udhcpc`.
//! This matches KOReader's approach. The rationale is:
//!
//! 1. wpa_supplicant handles 802.11 connectivity
//! 2. Network configuration is typically managed by the platform (Nickel/sysinit)
//! 3. Avoiding udhcpc reduces complexity and potential race conditions
//! 4. If DHCP is needed, it can be added later based on platform requirements
//!
//! ## Features Copied from KOReader
//!
//! The following improvements were adopted from KOReader's implementation:
//!
//! 1. **No udhcpc**: KOReader omits udhcpc, relying on wpa_supplicant and platform
//!    network management instead of a separate DHCP client
...
use crate::device::wifi::kobo::types::{PowerToggle, WifiModuleConfig};
...
impl KoboWifiManager {
    ...
    fn ioctl_wifi_ctrl(&self, fd: std::os::fd::RawFd, enable: u8) -> Result<(), WifiError> {
        let ioctl_arg: [u8; 4] = [enable, 0, 0, 0];

        let ret = unsafe {
            libc::ioctl(
                fd,
                NTX_IO_WIFI_CTRL as libc::c_ulong,
                ioctl_arg.as_ptr() as *mut libc::c_void,
            )
        };
    ...
    fn power_up_wmt(&self) -> Result<(), WifiError> {
        ...
        let wifi_module_path = format!("{}/{}.ko", module_path, self.config.module_name);
        if Path::new(&wifi_module_path).exists() {
            self.insmod_asneeded(&wifi_module_path, &self.config.module_name)?;
        }
    ...
    #[cfg_attr(feature = "otel", tracing::instrument(skip(self), ret(level=tracing::Level::TRACE)))]
    fn build_module_params(&self) -> String {
        let mut params = String::new();

        if let Some(country_code) = self.read_country_code() {
            match self.config.module_name.as_str() {
                "8821cs" => {
                    params.push_str(&format!(" rtw_country_code={}", country_code));
                }
                "moal" => {
                    params.push_str(&format!(" reg_alpha2={}", country_code));
                }
                _ => {}
            }
        }

        if self.config.module_name == "moal" {
            params.push_str(" mod_para=nxp/wifi_mod_para_sd8987.conf");
        }
    ...
    fn load_wifi_module(&self) -> Result<(), WifiError> {
        ...
        let wifi_module_path = if Path::new(&format!(
            "{}/{}/{}.ko",
            DRIVERS_DIR, platform, self.config.module_name
        ))
        .exists()
        {
            format!(
            ...
            format!("{}/{}.ko", self.config.module_path, self.config.module_name)
        };
        ...
        let full_cmd = format!("{}{}", wifi_module_path, module_params);
        let mut parts = full_cmd.split_whitespace();
        let path = parts.next().unwrap();
        let params: Vec<&str> = parts.collect();
        self.insmod_asneeded_with_params(path, &self.config.module_name, &params)?;
...
impl WifiManager for KoboWifiManager {
    ...
    fn enable(&self) -> Result<(), WifiError> {
        ...
        if is_module_loaded(&self.config.module_name) && is_interface_up(&self.config.interface) {
    ...
    fn disable(&self) -> Result<(), WifiError> {
        ...
        if self.config.power_toggle != PowerToggle::Wmt {
            self.rmmod(&self.config.module_name)?;
            if self.config.module_name == "moal" {
                self.rmmod("mlan")?;
            }
        }
