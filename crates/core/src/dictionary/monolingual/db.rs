...
impl Db {
    ...
    pub(super) async fn upsert_entry(
        &self,
        lang: &str,
    ...
    /// Returns an error if the database query fails.
    #[cfg_attr(feature = "tracing", tracing::instrument(skip(self)))]
    pub(super) async fn get_most_recent_cached_at(&self) -> Result<Option<UnixTimestamp>, Error> {
    ...
    /// Returns an error if the database query fails.
    #[cfg_attr(feature = "tracing", tracing::instrument(skip(self)))]
    pub(super) async fn list_installed_langs(&self) -> Result<Vec<String>, Error> {
        let rows =
    ...
    /// Returns an error if the database write fails.
    #[cfg_attr(feature = "tracing", tracing::instrument(skip(self), fields(lang = %lang)))]
    pub(super) async fn record_install(
        &self,
        lang: &str,
    ...
    /// Returns an error if the database write fails.
    #[cfg_attr(feature = "tracing", tracing::instrument(skip(self), fields(lang = %lang)))]
    pub(super) async fn remove_installed(&self, lang: &str) -> Result<(), Error> {
        sqlx::query!(
    ...
    pub(super) async fn is_update_available(&self, lang: &str) -> Result<bool, Error> {
        let result = sqlx::query_scalar!(
            r#"SELECT EXISTS(
                    SELECT 1
...
mod tests {
    ...
    async fn test_upsert_and_get_roundtrip() {
        ...
        let all = db.get_all_entries().await.expect("get_all should not fail");
        assert_eq!(all.len(), 1);
