...
impl MonolingualDictionaryService {
    ...
    /// moved aside before that rename and restored if publishing fails.
    /// Installation is recorded in the registry only after the dest exists.
    ///
    ...
    #[cfg_attr(
        feature = "tracing",
        tracing::instrument(skip(self, entry, progress_callback), fields(lang = %lang, include_etymologies))
    )]
    fn do_install<F>(
        &self,
        lang: &str,
        ...
        F: FnMut(u64, u64),
    {
        ...
        commit_extracted_dictionary(&dest, &staging, lang, &mut staging_guard)?;
        self.db.record_install(lang, entry.updated.into())?;
    ...
    ///
    /// Also deletes the language destination directory plus `.{lang}.partial` and
    /// `.{lang}.replaced` so startup reconcile cannot restore or re-register a
...
fn reconcile_reader_dict_tree(db: &Db, root: &Path) -> Result<(), MonolingualError> {
    ...
    let registered: HashSet<String> = db.list_installed_langs()?.into_iter().collect();
    tracing::debug!(
    ...
    for (path, lang) in stagings {
        if let Err(error) = reconcile_staging_dir(db, root, &path, &lang) {
            tracing::warn!(
                lang,
                path = %path.display(),
                error = %error,
    ...
    for (path, lang) in replaced {
        if let Err(error) = reconcile_replaced_dir(db, root, &path, &lang) {
            tracing::warn!(
                lang,
                path = %path.display(),
                error = %error,
    ...
    for (path, name) in dests {
        if has_dict_pair(&path) && !registered.contains(&name) {
            register_complete_install(db, &name)?;
        }
    }
...
#[cfg_attr(
    feature = "tracing",
    tracing::instrument(skip(db, root), fields(lang, staging = %staging.display()))
)]
...
#[cfg_attr(
    feature = "tracing",
    tracing::instrument(skip(db, root), fields(lang, replaced = %replaced.display()))
)]
fn reconcile_replaced_dir(
    ...
    if has_dict_pair(replaced) {
        if dest.exists() {
            fs::remove_dir_all(&dest)?;
            tracing::info!(
                lang,
                dest = %dest.display(),
                "removed incomplete dictionary dest before restoring replaced"
            );
        }
    ...
    if let Err(error) = fs::remove_dir_all(replaced) {
        tracing::warn!(
            path = %replaced.display(),
            error = %error,
            "failed to remove incomplete replaced dictionary directory"
        );
    }
    Ok(())
}

#[cfg_attr(feature = "tracing", tracing::instrument(skip(db), fields(lang)))]
...
fn staging_lang(name: &str) -> Option<&str> {
    name.strip_prefix('.')?.strip_suffix(STAGING_SUFFIX)
}

fn replaced_lang(name: &str) -> Option<&str> {
    name.strip_prefix('.')?.strip_suffix(REPLACED_SUFFIX)
}

#[cfg_attr(
    feature = "tracing",
    tracing::instrument(skip(staging_guard), fields(lang, dest = %dest.display(), staging = %staging.display()))
)]
fn commit_extracted_dictionary(
    ...
    publish_extracted_dictionary(dest, staging, lang, staging_guard)?;
    Ok(())
}
...
mod tests {
    ...
    #[test]
    fn test_publish_replaces_existing_dest_and_removes_aside() {
        ...
        let mut guard = crate::fs::RemovePathOnDrop::dir(staging.clone());
        publish_extracted_dictionary(&dest, &staging, "en", &mut guard).unwrap();
    ...
    #[test]
    fn test_reconcile_restores_replaced_aside_when_dest_missing() {
        let (service, dir, _db) = create_test_service();
        let root = dir.path().join(READER_DICT_SUBDIR);
        let aside = replaced_dir(&root, "en");
        fs::create_dir_all(&aside).unwrap();
