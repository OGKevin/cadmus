...
impl OtaClient {
        ...
        F: FnMut(OtaProgress),
    {
        ...
        let artifact = self
            ...
            .map_err(|_| OtaError::NoArtifacts(pr_number))?;
    ...
    /// 1. Verifies sufficient disk space (100MB required)
    /// 2. Queries GitHub API for the latest successful "Cargo" workflow run on `master`
    /// 3. Locates artifacts matching "cadmus-kobo-{sha}" pattern (or "cadmus-kobo-test-{sha}" with `test` feature)
    /// 4. Downloads the artifact ZIP file to `/tmp/cadmus-ota-latest.zip`
    ///
    /// # Arguments
    ///
    /// * `progress_callback` - Function called with progress updates during download
        ...
        F: FnMut(OtaProgress),
    {
        ...
        let runs_url = format!(
            "https://api.github.com/repos/ogkevin/cadmus/actions/runs?branch={}&event=push&status=success&per_page=10",
            default_branch
        );
        tracing::debug!(url = %runs_url, "Fetching default branch workflow runs");
        ...
        let response = response.error_for_status().map_err(|e| {
            tracing::error!(
                status = ?e.status(),
                error = %e,
                "Default branch workflow runs fetch failed"
            );
            OtaError::Api(format!("Failed to fetch workflow runs: {}", e))
        })?;
        ...
        let cargo_run = runs
            .workflow_runs
            .iter()
            .find(|r| r.name == "Cargo" && r.conclusion.as_deref() == Some("success"))
            .ok_or_else(|| {
    ...
    pub fn extract_and_deploy(&self, zip_path: PathBuf) -> Result<PathBuf, OtaError> {
        ...
        let temp_dir = std::env::temp_dir().join("cadmus-ota-extract");
        std::fs::create_dir_all(&temp_dir)?;

        let extracted_path = temp_dir.join(kobo_root_name);
        let mut extracted_file = File::create(&extracted_path)?;
        extracted_file.write_all(&kobo_root_data)?;

        tracing::debug!(
            path = ?extracted_path,
            "Extracted KoboRoot to temporary location"
        );

        self.deploy(extracted_path)
    }
